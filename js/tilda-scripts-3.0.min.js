...
(window.isSearchBot = !1),
  ...
  t_onReady(function () {
    ...
    let modalForm = document.querySelector("#form651950890");
    if (modalForm) {
      ...
      async function sendData(data) {
        return await fetch(`/wp-content/themes/citydepil/send_mail.php`, {
          method: "POST",
          body: data,
        });
      }
      ...
      async function formSubmit() {
        let name = modalForm.elements["Name"].value.trim();
        let phone = modalForm.elements["Phone"].value.trim();
        let checkbox = modalForm.elements["Checkbox12312312"].checked;

        if (name === "" || phone === "" || !checkbox) {
          alert(
            "Пожалуйста, заполните все обязательные поля и отметьте чекбокс."
          );
          return;
        }

        let data = serializeForm(modalForm);
